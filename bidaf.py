...
class BiDAF(object):
    def __init__(self,embeddings,question_len,context_len,embedding_size,hidden_size,keep_prob,stddev=0.1): #stddev=0.1
        self.embeddings = embeddings
        self.que_time_step  = question_len
        self.con_time_step  = context_len
    ...
    def attention_flow_layer(self,H,U):
        with tf.variable_scope('Attention_flow_layer'):
            ...
            b = tf.nn.softmax(tf.reduce_max(S,2),-1) #先在column上使用maximum function，再softmax
            # (B,T) column上的维度被压缩了
            ...
            G = tf.concat([H_trans,attended_U,h_el_mul_att_u,h_el_mul_att_h],axis=1) # (B,8d,T)
    ...
    def output_layer(self,G,M,con_time_step,hidden_size,embedding_size,keep_prob):
        with tf.variable_scope('Output_layer'):
            ...
            p1 = tf.reshape(p1,[-1,M.get_shape().as_list()[-1]]) # (?,con_time_step)
            ...
            p2 = tf.reshape(p2,[-1,M.get_shape().as_list()[-1]]) # (?,con_time_step)
